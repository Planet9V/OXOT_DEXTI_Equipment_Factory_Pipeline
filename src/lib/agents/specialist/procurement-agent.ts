...
export class ProcurementAgent extends BaseSpecialist<ProcurementInput, ProcurementOutput> {
    ...
    async execute(input: ProcurementInput, runId: string): Promise<ProcurementOutput> {
        ...
        // We dynamically inject context into the system prompt for this execution
        // using the base systemPrompt but not mutating the class-level config.
        const dynamicSystemPrompt = this.config.systemPrompt
            .replace(/\[REFERENCE_EQUIPMENT_JSON\]/g, () => JSON.stringify(equipment, null, 2))
            .replace(/\[REFERENCE_TAG\]/g, () => equipment.tag || 'REF');
...
Ensure the models are compatible with the specifications provided in the context.`;
        ...
        // Provide the dynamic system prompt as additional instructions if base class
        // doesn't support overriding system prompt directly (callLLM normally uses config).
        // If your BaseSpecialist has an override for systemPrompt in callLLM, use it:
        // const response = await this.callLLM(userMessage, { systemPrompt: dynamicSystemPrompt });
        // Assuming we need to construct it inline, we'll temporarily swap it or construct a full user payload.

        const originalSystemPrompt = this.config.systemPrompt;
        this.config.systemPrompt = dynamicSystemPrompt;

        let response;
        try {
            response = await this.callLLM(userMessage);
        } finally {
