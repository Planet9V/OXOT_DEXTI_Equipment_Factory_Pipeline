...
export class ProcurementAgent extends BaseSpecialist<ProcurementInput, ProcurementOutput> {
    ...
    async execute(input: ProcurementInput, runId: string): Promise<ProcurementOutput> {
        ...
        let userMessage = SYSTEM_PROMPT.replace('[REFERENCE_EQUIPMENT_JSON]', JSON.stringify(equipment, null, 2));
        userMessage = userMessage.replace('[REFERENCE_TAG]', String(equipment.tag || 'REF'));
        ...
        // We strictly enforce returning ONLY a valid JSON array.
