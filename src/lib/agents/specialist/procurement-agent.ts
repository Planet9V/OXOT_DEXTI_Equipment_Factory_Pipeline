...
export class ProcurementAgent extends BaseSpecialist<ProcurementInput, ProcurementOutput> {
    ...
    async execute(input: ProcurementInput, runId: string): Promise<ProcurementOutput> {
        ...
        const userMessage = `Find 3 distinct real-world vendor models for the following Reference Equipment:`;
        const additionalInstructions = `Context: ${JSON.stringify(equipment, null, 2)}`;

        // Pass dynamic context as an override parameter instead of mutating global config.
        const dynamicPrompt = SYSTEM_PROMPT
            .replace('[REFERENCE_EQUIPMENT_JSON]', () => JSON.stringify(equipment, null, 2))
            .replace('[REFERENCE_TAG]', () => String(equipment.tag || 'REF'));

        // Call the LLM with tools
        const response = await this.callLLM(userMessage, dynamicPrompt);
