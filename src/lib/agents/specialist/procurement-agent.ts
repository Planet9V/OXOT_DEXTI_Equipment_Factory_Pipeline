...
export class ProcurementAgent extends BaseSpecialist<ProcurementInput, ProcurementOutput> {
    ...
    async execute(input: ProcurementInput, runId: string): Promise<ProcurementOutput> {
        ...
        const systemPromptOverride = SYSTEM_PROMPT
            .replace(/\[REFERENCE_EQUIPMENT_JSON\]/g, () => JSON.stringify(equipment, null, 2))
            .replace(/\[REFERENCE_TAG\]/g, equipment.tag || 'Unknown');

        // Use concise user message
        const userMessage = `Find 3 distinct real-world vendor models for the provided Reference Equipment. Ensure the models are compatible with the specifications.`;
