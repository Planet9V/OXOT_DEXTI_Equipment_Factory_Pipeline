...
const SYSTEM_PROMPT = `You are "The Procurement Officer," responsible for sourcing specific vendor equipment.
...
export class ProcurementAgent extends BaseSpecialist<ProcurementInput, ProcurementOutput> {
    ...
    async execute(input: ProcurementInput, runId: string): Promise<ProcurementOutput> {
        ...
        // Apply dynamic string replacements locally to construct the prompt override
        ...
        dynamicSystemPrompt = dynamicSystemPrompt.replace(/\[REFERENCE_TAG\]/g, equipment.tag || 'REF');

        const userMessage = `Find 3 distinct real-world vendor models for this Reference Equipment. Ensure the models are compatible with the specifications provided in the context.`;

        // Call the LLM with tools, overriding the base system prompt locally to prevent mutating shared state
        const response = await this.callLLM(userMessage, dynamicSystemPrompt);

        // Parse and validate the response
        // Ensure strict adherence to output format (JSON Array)
