...
Return JSON:
{
  ...
  "recommendations": ["..."]
     ...
     *
     * @param listOfTypes - Array of equipment types from the registry.
     * @returns Array of generated equipment cards.
     */
    async generateEquipmentCards(listOfTypes: string[]): Promise<Record<string, unknown>[]> {
        const context: AgentContext = {
            listOfTypesFromRegistry: listOfTypes.join('\n'),
        };

        const result = await this.chat(
            [{ role: 'user', content: 'Generate the DEXPI 2.0 equipment cards as instructed.' }],
            'theEngineer',
            context
        );
        ...
        if (context) {
            if (context.listOfTypesFromRegistry) {
                prompt = prompt.replace(
                    ...
                    () => context.listOfTypesFromRegistry!
                );
            }
