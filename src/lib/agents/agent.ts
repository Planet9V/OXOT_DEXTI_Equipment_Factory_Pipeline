...
Task: Generate **Full-Fidelity** DEXPI 2.0 equipment cards for the following list of equipment types provided in the context.
...
Return ONLY valid JSON, no markdown.`;
     ...
     */
    async generateFullFidelityCards(equipmentTypes: any[]): Promise<any[]> {
        const typesListStr = JSON.stringify(equipmentTypes, null, 2);

        const result = await this.chat(
            [{ role: 'user', content: `Please generate cards for the following equipment types: ${typesListStr}` }],
            'theEngineer',
            { additionalInstructions: `Equipment List:\n${typesListStr}` }
        );
        ...
        try {
            ...
            console.warn('[agent] Failed to parse full-fidelity cards JSON:', err);
        }
