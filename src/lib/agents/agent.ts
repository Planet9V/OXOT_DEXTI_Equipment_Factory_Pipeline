...
Return ONLY valid JSON, no markdown.`;
    ...
    /**
     * Generates DEXPI 2.0 equipment cards for a list of equipment types.
     *
     * @param types - Array of equipment types to generate cards for.
     * @returns Array of generated equipment cards.
     */
    async generateEquipmentCards(types: string[]): Promise<Record<string, unknown>[]> {
    ...
    private buildSystemPrompt(persona: PersonaName, context?: AgentContext): string {
        ...
        if (context) {
            if (context.listOfTypesFromRegistry) {
                const typesJson = JSON.stringify(context.listOfTypesFromRegistry, null, 2);
                prompt = prompt.replace(/\[LIST_OF_TYPES_FROM_REGISTRY\]/g, () => typesJson);
            }
            if (context.referenceEquipment) {
                const equipmentJson = JSON.stringify(context.referenceEquipment, null, 2);
