...
Return ONLY valid JSON, no markdown.`;
     ...
     * @returns Array of fully-fidelity equipment cards.
     */
    async generateEquipmentCards(equipmentTypes: string[]): Promise<Record<string, unknown>[]> {
        const typesList = equipmentTypes.map(t => `- ${t}`).join('\n');

        const prompt = `Generate equipment cards for the following items:\n${typesList}`;

        const result = await this.chat(
            [{ role: 'user', content: prompt }],
            'theEngineer',
            { additionalInstructions: `Context: Please replace [LIST_OF_TYPES_FROM_REGISTRY] with:\n${typesList}` }
        );
