...
Return ONLY valid JSON, no markdown.`;
    ...
    async findVendorVariations(referenceEquipment: Record<string, unknown>): Promise<VendorVariation[]> {
        // Build the prompt dynamically to replace the placeholders, ensuring the full schema constraints are maintained.
        // We inject the correct referenceId placeholder using the actual tag.
        const systemPromptStr = PERSONAS.procurementOfficer
            .replace('[REFERENCE_EQUIPMENT_JSON]', JSON.stringify(referenceEquipment, null, 2))
            .replace('[REFERENCE_TAG]', (referenceEquipment.tag as string) || 'REF');

        const prompt = systemPromptStr + '\n- Return ONLY valid JSON array, no markdown.';
