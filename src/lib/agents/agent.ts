...
Return ONLY valid JSON, no markdown.`;
    ...
    /**
     * Generates a comprehensive registry of unique equipment types for a specific sector.
     *
     * @param sector - CISA Sector Name (e.g., "Oil & Gas").
     * @param subSector - Sub-sector Name (e.g., "PWR").
     * @param facility - Facility details or constraints.
     * @returns Parsed JSON object representing the equipment registry.
     */
    async generateEquipmentRegistry(sector: string, subSector: string, facility: string): Promise<Record<string, unknown>> {
        let prompt = PERSONAS.theSurveyor;
        prompt = prompt.replace('[SECTOR NAME]', sector);
        prompt = prompt.replace('[SECTOR_CODE]', sector.substring(0, 4).toUpperCase());
        prompt = prompt.replace('[SUB_SECTOR_CODE]', subSector.toUpperCase());

        const contextualPrompt = `
${prompt}

Target Facility Context: ${facility}
`;
        ...
        return { sector: sector.substring(0, 4).toUpperCase(), subSector: subSector.toUpperCase(), equipment: [] };
    }

    /**
     * Generates Full-Fidelity DEXPI 2.0 equipment cards for a list of equipment types.
     *
     * @param equipmentTypes - Array of equipment type definitions from a registry.
     * @returns Array of generated equipment cards.
     */
    async generateReferenceCards(equipmentTypes: any[]): Promise<Record<string, unknown>[]> {
        const prompt = PERSONAS.theEngineer;
        const typesJson = JSON.stringify(equipmentTypes, null, 2);

        const contextualPrompt = prompt.replace('[LIST_OF_TYPES_FROM_REGISTRY]', typesJson);
        ...
        try {
            ...
            console.warn('[agent] Failed to parse generated reference cards JSON');
        }
