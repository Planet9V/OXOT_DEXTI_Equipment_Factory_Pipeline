...
Return ONLY valid JSON, no markdown.`;
    ...
    /**
     * Generates a comprehensive registry of unique equipment types for a specific CISA sector.
     *
     ...
     * @returns A parsed JSON object containing the registry.
     */
    async generateEquipmentRegistry(sectorName: string, sectorCode: string, subSectorCode: string): Promise<any> {
        const prompt = PERSONAS.theSurveyor
            .replace('[SECTOR NAME]', sectorName)
            .replace('[SECTOR_CODE]', sectorCode)
            .replace('[SUB_SECTOR_CODE]', subSectorCode);

        const result = await this.chat([{ role: 'user', content: prompt }], 'theSurveyor');
     ...
     *
     * @param equipmentTypes - An array of equipment type objects from the registry.
     * @returns A parsed JSON array containing the generated cards.
     */
    async generateReferenceCards(equipmentTypes: any[]): Promise<any[]> {
        const equipmentTypesJson = JSON.stringify(equipmentTypes, null, 2);
        const prompt = PERSONAS.theEngineer.replace('[LIST_OF_TYPES_FROM_REGISTRY]', equipmentTypesJson);

        const result = await this.chat([{ role: 'user', content: prompt }], 'theEngineer');
