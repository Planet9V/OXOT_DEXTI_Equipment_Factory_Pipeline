...
Return JSON:
{
  ...
  "recommendations": ["..."]
    ...
    /**
     * Generates Full-Fidelity DEXPI 2.0 equipment cards using The Engineer persona.
     * @param equipmentTypes Array of equipment type names to generate cards for.
     * @returns Array of generated equipment cards.
     */
    async generateEquipmentCards(equipmentTypes: string[]): Promise<any[]> {
        const typesList = equipmentTypes.map(t => `- ${t}`).join('\n');

        const response = await this.chat(
            [
                { role: 'user', content: `Please generate the Full-Fidelity DEXPI 2.0 equipment cards for the following types:\n${typesList}\nEnsure you strictly output a JSON array of objects and nothing else.` }
            ],
            'theEngineer',
            { additionalInstructions: `Replace [LIST_OF_TYPES_FROM_REGISTRY] with:\n${typesList}` }
        );
        ...
        try {
            ...
            console.error('Failed to parse equipment cards JSON', error);
            return [];
        }
    }
