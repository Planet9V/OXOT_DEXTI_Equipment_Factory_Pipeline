...
Return ONLY valid JSON, no markdown.`;
    ...
    /**
     * Generates Full-Fidelity DEXPI 2.0 equipment cards.
     *
    ...
    async generateEquipmentCards(equipmentTypes: string[]): Promise<Record<string, unknown>[]> {
        ...
        const context: AgentContext = {
            additionalInstructions: `Context: [LIST_OF_TYPES_FROM_REGISTRY] = \n${typesList}`
        };

        const result = await this.chat(
            [{ role: 'user', content: 'Generate DEXPI 2.0 equipment cards for the requested types.' }],
            'theEngineer',
            context
        );
