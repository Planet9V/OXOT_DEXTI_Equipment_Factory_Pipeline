...
Ensure the models are compatible with the specifications provided in the context. Return ONLY a valid JSON array.`;
        ...
        } as any; // any to bypass AgentContext typing temporarily if it doesn't include referenceEquipment
...
Return JSON:
{
  ...
  "recommendations": ["..."]
     ...
     */
    private buildSystemPrompt(persona: PersonaName, context?: AgentContext & Record<string, any>): string {
        let prompt = PERSONAS[persona];

        if (context) {
            if (prompt.includes('[REFERENCE_EQUIPMENT_JSON]') && context.referenceEquipment) {
                prompt = prompt.replace(/\[REFERENCE_EQUIPMENT_JSON\]/g, () => JSON.stringify(context.referenceEquipment, null, 2));
            }
            if (prompt.includes('[REFERENCE_TAG]') && context.referenceTag) {
                prompt = prompt.replace(/\[REFERENCE_TAG\]/g, context.referenceTag);
            }
            if (prompt.includes('[SECTOR NAME]') && context.sectorName) {
                prompt = prompt.replace(/\[SECTOR NAME\]/g, context.sectorName);
            }
            if (prompt.includes('[SECTOR_CODE]') && context.sectorCode) {
                prompt = prompt.replace(/\[SECTOR_CODE\]/g, context.sectorCode);
            }
            if (prompt.includes('[SUB_SECTOR_CODE]') && context.subSectorCode) {
