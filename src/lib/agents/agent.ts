...
Return ONLY valid JSON, no markdown.`;
    ...
    async findVendorVariations(referenceEquipment: Record<string, unknown>): Promise<VendorVariation[]> {
        const prompt = `Find 3 distinct real-world vendor models for the Reference Equipment provided in your context.`;
  ...
  "recommendations": ["..."]
    ...
    private buildSystemPrompt(persona: PersonaName, context?: AgentContext): string {
        ...
        if (context) {
            ...
            if (context.sectorName) parts.push(`Sector Name: ${context.sectorName}`);
            if (context.sectorCode) parts.push(`Sector Code: ${context.sectorCode}`);
            if (context.subSectorCode) parts.push(`Sub-Sector Code: ${context.subSectorCode}`);
