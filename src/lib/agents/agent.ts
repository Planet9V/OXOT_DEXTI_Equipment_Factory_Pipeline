...
Return ONLY valid JSON, no markdown.`;
     ...
     *
     * @param equipmentTypes - The list of equipment types to generate cards for.
     * @returns Array of generated equipment cards.
     */
    async generateEquipmentCards(equipmentTypes: Record<string, unknown>[] | string): Promise<Record<string, unknown>[]> {
        const typesStr = typeof equipmentTypes === 'string'
            ? equipmentTypes
            : JSON.stringify(equipmentTypes, null, 2);

        const result = await this.chat(
            [{ role: 'user', content: 'Please generate the DEXPI 2.0 equipment cards.' }],
            'theEngineer',
            { additionalInstructions: `[LIST_OF_TYPES_FROM_REGISTRY]\n${typesStr}` }
        );
