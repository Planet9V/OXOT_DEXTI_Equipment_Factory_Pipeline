...
Return ONLY valid JSON, no markdown.`;
    ...
    /**
     * Generates a comprehensive registry of unique equipment types for a given sector.
     *
     * @param sectorName - The name of the sector (e.g., Oil & Gas, Water Treatment).
     * @param sectorCode - The CISA sector code.
     * @param subSectorCode - The sub-sector code.
     * @returns Parsed JSON object representing the registry.
     */
    async generateEquipmentRegistry(sectorName: string, sectorCode: string, subSectorCode: string): Promise<Record<string, unknown>> {
        const systemPromptTemplate = PERSONAS['theSurveyor'];
        const customizedPrompt = systemPromptTemplate
            .replace('[SECTOR NAME]', sectorName)
            .replace('[SECTOR_CODE]', sectorCode)
            .replace('[SUB_SECTOR_CODE]', subSectorCode);

        // We override the system prompt by passing it as the first user message,
        // and using 'coordinator' (or any) persona as base, since `chat` prepends the system prompt.
        ...
        return { sector: sectorCode, subSector: subSectorCode, equipment: [] };
    }

    /**
     * Generates Full-Fidelity DEXPI 2.0 equipment cards for a list of equipment types.
     *
     * @param equipmentTypes - Array of equipment types to generate cards for.
     * @returns Array of parsed JSON objects representing the equipment cards.
     */
    async generateReferenceCards(equipmentTypes: string[]): Promise<Record<string, unknown>[]> {
        const listString = equipmentTypes.map(t => `- ${t}`).join('\n');

        // As noticed in the code review, injecting via `theEngineer` persona directly will duplicate prompt instructions.
        // The safest approach here, since we are using chat, is to supply the prompt as a user message
        // and run under a general persona to avoid duplicating the huge prompt inside the system message un-replaced.
        const systemPromptTemplate = PERSONAS['theEngineer'];
        const customizedPrompt = systemPromptTemplate.replace('[LIST_OF_TYPES_FROM_REGISTRY]', listString);
        ...
        try {
            ...
            if (jsonMatch) {
                return JSON.parse(jsonMatch[0]) as Record<string, unknown>[];
                 ...
                 // Try parsing a single object if it failed to return an array
                 const singleMatch = result.content.match(/\{[\s\S]*\}/);
                 if (singleMatch) {
            ...
            console.warn('[agent] Failed to parse reference cards JSON', e);
        }
