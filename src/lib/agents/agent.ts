...
- Return ONLY valid JSON array, no markdown.`;
     ...
     *
     * @param equipmentTypes - List of equipment types to generate cards for.
     * @param context - Optional context to guide generation.
     * @returns Array of fully generated equipment cards.
     */
    async generateEquipmentCards(equipmentTypes: string[], context?: AgentContext): Promise<Record<string, unknown>[]> {
        const fullContext: AgentContext = {
            ...context,
            listOfTypesFromRegistry: equipmentTypes,
        };

        const result = await this.chat(
            [{ role: 'user', content: 'Generate DEXPI 2.0 equipment cards for the provided list of equipment types.' }],
            'theEngineer',
            fullContext
        );
    ...
    private buildSystemPrompt(persona: PersonaName, context?: AgentContext): string {
        ...
        if (context) {
            if (context.listOfTypesFromRegistry) {
                prompt = prompt.replace(/\[LIST_OF_TYPES_FROM_REGISTRY\]/g, () => JSON.stringify(context.listOfTypesFromRegistry));
            }
            if (context.referenceEquipment) {
                prompt = prompt.replace(/\[REFERENCE_EQUIPMENT_JSON\]/g, () => JSON.stringify(context.referenceEquipment));
            }
