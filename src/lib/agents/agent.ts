...
Return ONLY valid JSON, no markdown.`;
    ...
    async findVendorVariations(referenceEquipment: Record<string, unknown>): Promise<VendorVariation[]> {
        const prompt = `Find 3 distinct real-world vendor models for the Reference Equipment provided in the context. Return ONLY a valid JSON array.`;

        // Inject dynamic context properly
        const context: AgentContext = {
            additionalInstructions: `Context: ${JSON.stringify(referenceEquipment, null, 2)}`
        };
