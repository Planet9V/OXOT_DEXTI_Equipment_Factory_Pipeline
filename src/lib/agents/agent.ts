...
Return ONLY valid JSON, no markdown.`;
    ...
    async findVendorVariations(referenceEquipment: Record<string, unknown>): Promise<VendorVariation[]> {
        const result = await this.chat(
            [{ role: 'user', content: 'Generate vendor variations.' }],
            'procurementOfficer',
            { referenceEquipment }
        );
    ...
    private buildSystemPrompt(persona: PersonaName, context?: AgentContext): string {
        ...
        if (context) {
            if (context.referenceEquipment) {
                prompt = prompt
                    .replace('[REFERENCE_EQUIPMENT_JSON]', JSON.stringify(context.referenceEquipment, null, 2))
                    .replace(/\[REFERENCE_TAG\]/g, String((context.referenceEquipment as any).tag || 'REF'));
            }
