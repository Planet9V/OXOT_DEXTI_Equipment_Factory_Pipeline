...
Return JSON:
{
  ...
  "improvedCard": { /* only if score < 80 */ }
    ...
    /**
     * Generate Full-Fidelity DEXPI 2.0 equipment cards for a list of equipment types.
     *
     * @param equipmentTypes - Array of equipment types from the registry.
     * @returns Array of fully-compliant DEXPI 2.0 equipment cards.
     */
    async generateEquipmentCards(equipmentTypes: Array<Record<string, unknown>>): Promise<Array<Record<string, unknown>>> {
        const context: AgentContext = {
            listOfTypesFromRegistry: JSON.stringify(equipmentTypes, null, 2),
        };

        const result = await this.chat(
            [{ role: 'user', content: 'Generate the equipment cards according to the prompt schema.' }],
            'theEngineer',
            context,
        );
    ...
    private buildSystemPrompt(persona: PersonaName, context?: AgentContext): string {
        ...
        if (context) {
            if (context.listOfTypesFromRegistry) {
                prompt = prompt.replace(/\[LIST_OF_TYPES_FROM_REGISTRY\]/g, () => context.listOfTypesFromRegistry as string);
            }
            if (context.referenceEquipment) {
                prompt = prompt.replace(/\[REFERENCE_EQUIPMENT_JSON\]/g, () => context.referenceEquipment as string);
            }
            if (context.referenceTag) {
                prompt = prompt.replace(/\[REFERENCE_TAG\]/g, () => context.referenceTag as string);
            }
            if (context.sectorName) {
                prompt = prompt.replace(/\[SECTOR NAME\]/g, () => context.sectorName as string);
            }
            if (context.sectorCode) {
                prompt = prompt.replace(/\[SECTOR_CODE\]/g, () => context.sectorCode as string);
            }
            if (context.subSectorCode) {
                prompt = prompt.replace(/\[SUB_SECTOR_CODE\]/g, () => context.subSectorCode as string);
            }
