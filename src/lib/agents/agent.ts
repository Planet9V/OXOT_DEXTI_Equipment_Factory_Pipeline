...
Return JSON:
{
  ...
  "recommendations": ["..."]
    ...
    /**
     * Generate an equipment registry using theSurveyor persona.
     *
     * @param context - Agent context with sector details.
    ...
    async generateEquipmentRegistry(context: AgentContext): Promise<Record<string, unknown> | null> {
        const result = await this.chat(
            [{ role: 'user', content: 'Begin registry generation.' }],
            'theSurveyor',
            context
        );
        ...
        try {
            ...
            console.warn('[agent] Failed to parse generated registry JSON', err);
            return null;
        }
    }
    ...
    private buildSystemPrompt(persona: PersonaName, context?: AgentContext): string {
        ...
        if (context) {
            ...
            if (context.referenceEquipment) {
                prompt = prompt.replace(/\[LIST_OF_TYPES_FROM_REGISTRY\]/g, context.referenceEquipment);
