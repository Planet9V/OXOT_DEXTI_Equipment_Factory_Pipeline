...
Return JSON:
{
  ...
  "recommendations": ["..."]
    ...
    private buildSystemPrompt(persona: PersonaName, context?: AgentContext): string {
        ...
        if (context) {
            if (context.referenceEquipment) {
                prompt = prompt.replace(/\[REFERENCE_EQUIPMENT_JSON\]/g, () =>
                    typeof context.referenceEquipment === 'string'
    ...
    /**
     * Generate equipment cards using theEngineer persona.
     *
     * @param typesList - List of equipment types from the registry to generate cards for.
     * @returns Array of generated equipment cards.
     */
    async generateEquipmentCards(typesList: any[]): Promise<Record<string, unknown>[]> {
        const context: AgentContext = {
            listOfTypesFromRegistry: JSON.stringify(typesList, null, 2),
        };

        const prompt = `Please generate equipment cards for the provided list of types.`;
