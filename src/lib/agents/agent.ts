...
Return ONLY valid JSON, no markdown.`;
    ...
    /**
     * Generate an equipment registry for a given sector using The Surveyor.
     *
    ...
    async generateEquipmentRegistry(sectorName: string, sectorCode: string, subSectorCode: string): Promise<Record<string, unknown> | null> {
        const result = await this.chat(
            [{ role: 'user', content: 'Generate the equipment registry.' }],
            'theSurveyor',
            { sectorName, sectorCode, subSectorCode }
        );
...
Return JSON:
{
  ...
  "recommendations": ["..."]
    ...
    private buildSystemPrompt(persona: PersonaName, context?: AgentContext): string {
        ...
        if (context) {
            if (context.sectorName) prompt = prompt.replace('[SECTOR NAME]', context.sectorName);
            if (context.sectorCode) prompt = prompt.replace('[SECTOR_CODE]', context.sectorCode);
            if (context.subSectorCode) prompt = prompt.replace('[SUB_SECTOR_CODE]', context.subSectorCode);
