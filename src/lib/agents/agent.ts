...
Return JSON:
{
  ...
  "recommendations": ["..."]
    ...
    /**
     * Generates a list of DEXPI 2.0 compliant equipment cards from a list of equipment types.
     *
     * @param listOfTypes - Array of equipment types to generate cards for.
     * @returns Array of generated equipment cards.
     */
    async generateEquipmentCards(listOfTypes: string[]): Promise<any[]> {
        const context: AgentContext = {
            listOfTypesFromRegistry: listOfTypes,
        };

        const result = await this.chat(
            [{ role: 'user', content: 'Generate equipment cards for the provided list.' }],
            'theEngineer',
            context
        );
