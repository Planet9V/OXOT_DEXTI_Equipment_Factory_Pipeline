...
Return ONLY valid JSON, no markdown.`;
     ...
     *
     * @param sector - The sector name (e.g., 'Nuclear').
     * @param subSector - The sub-sector name.
     * @param customPrompt - Optional custom prompt or context.
     * @returns Parsed JSON equipment registry.
     */
    async generateEquipmentRegistry(sector: string, subSector: string, customPrompt?: string): Promise<any> {
        let promptContext = customPrompt || `Task: Create a comprehensive registry of unique equipment types for the ${sector} sector.`;
        promptContext = promptContext.replace('[SECTOR_CODE]', sector).replace('[SUB_SECTOR_CODE]', subSector).replace('[SECTOR NAME]', sector);
        ...
        return { sector, subSector, equipment: [] };
    }


    /**
     * Generate Full-Fidelity DEXPI 2.0 equipment cards for a list of equipment types using The Engineer persona.
     *
     * @param equipmentTypes - The list of equipment types.
     * @returns Parsed JSON array of DEXPI equipment cards.
     */
    async generateReferenceCards(equipmentTypes: any[]): Promise<any[]> {
        const promptContext = `Task: Generate **Full-Fidelity** DEXPI 2.0 equipment cards for the following list of equipment types:
