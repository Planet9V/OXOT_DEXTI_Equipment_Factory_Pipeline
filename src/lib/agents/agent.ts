...
Return JSON:
{
  ...
  "recommendations": ["..."]
    ...
    private buildSystemPrompt(persona: PersonaName, context?: AgentContext): string {
        ...
        if (context) {
            // Apply dynamic string replacements if placeholders exist in the prompt
            ...
            if (context.sector) {
                prompt = prompt.replace(/\[SECTOR_CODE\]/g, context.sector);
            }
            if (context.subSector) {
                prompt = prompt.replace(/\[SUB_SECTOR_CODE\]/g, context.subSector);
            }
            if (context.listOfTypesFromRegistry) {
                prompt = prompt.replace(/\[LIST_OF_TYPES_FROM_REGISTRY\]/g, () => JSON.stringify(context.listOfTypesFromRegistry, null, 2));
            }
            ...
            if (parts.length > 0 && !prompt.includes('[REFERENCE_EQUIPMENT_JSON]')) {
                // Only append legacy context block if we aren't doing direct replacements
                prompt += `\n\n## Current Context\n${parts.join('\n')}`;
            }
