...
Return ONLY valid JSON, no markdown.`;
    ...
    /**
     * Generate a registry of unique equipment types for a specific sector.
     *
     * @param sector - Sector name (e.g., Oil & Gas, Water Treatment).
     * @param sectorCode - Sector code (e.g., ENER, WATR).
     * @param subSectorCode - Sub-sector code (e.g., OG, TREATMENT).
     * @returns Parsed JSON object containing the registry.
     */
    async generateEquipmentRegistry(sector: string, sectorCode: string, subSectorCode: string): Promise<Record<string, unknown> | null> {
        const prompt = `Role: You are "The Surveyor," a senior industrial engineer mapping critical infrastructure assets.

Task: Create a comprehensive registry of unique equipment types for the ${sector} sector.
Focus on:
1.  Core Process Equipment (Pumps, Compressors, Reactors, Heat Exchangers)
2.  Support Systems (Valves, Tanks, Filters)
3.  Instrumentation (Flow, Level, Pressure, Temperature)
...
Constraint: List at least 50 unique types. Do not invent non-existent types. Use standard industry terminology.`;
    ...
    /**
     * Generate Full-Fidelity DEXPI 2.0 reference cards for a list of equipment types.
     *
     * @param equipmentTypes - List of equipment types from the registry.
     * @returns Array of parsed JSON objects containing the equipment cards.
     */
    async generateReferenceCards(equipmentTypes: any[]): Promise<any[]> {
        const prompt = `Role: You are "The Engineer," a detailed mechanical specification expert.

Task: Generate **Full-Fidelity** DEXPI 2.0 equipment cards for the following list of equipment types:
${JSON.stringify(equipmentTypes, null, 2)}
...
- Return a JSON array of objects.`;
        ...
        try {
            ...
            console.warn('[agent] Failed to parse reference cards JSON');
        }
