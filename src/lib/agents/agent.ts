...
- Return ONLY valid JSON array, no markdown.`;
    ...
    /**
     * Generate an equipment registry for a specific sector using The Surveyor.
     *
     * @param sectorName - Full name of the sector (e.g., "Nuclear Power").
     * @param sectorCode - Short code for the sector (e.g., "NUCL").
     * @param subSectorCode - Short code for the sub-sector (e.g., "PWR").
     * @param customPrompt - Optional specific instructions (like "Nuclear Power (PWR Primary Loop)").
    ...
    async generateEquipmentRegistry(
        ...
        let prompt = customPrompt || `Task: Create a comprehensive registry of unique equipment types for the ${sectorName} sector.
Output Format: Standard JSON Registry format using sector "${sectorCode}" and subSector "${subSectorCode}".
Constraint: List at least 50 unique types. Do not invent non-existent types. Use standard industry terminology.`;
    ...
    /**
     * Generate Full-Fidelity DEXPI 2.0 reference cards for given equipment types.
     *
     * @param equipmentTypes - Array of equipment type definitions from registry.
     * @returns Array of generated reference cards.
     */
    async generateReferenceCards(equipmentTypes: Record<string, unknown>[]): Promise<Record<string, unknown>[]> {
