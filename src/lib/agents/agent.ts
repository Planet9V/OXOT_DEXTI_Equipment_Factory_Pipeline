...
Return ONLY valid JSON, no markdown.`;
    ...
    /**
     * Generates Full-Fidelity DEXPI 2.0 equipment cards from a list of equipment types.
     *
     * @param listOfTypesFromRegistry - Array of equipment types.
     * @param context - Optional context to supply additional information.
     * @returns Array of generated equipment cards.
     */
    async generateEquipmentCards(listOfTypesFromRegistry: string[], context?: AgentContext): Promise<Record<string, unknown>[]> {
        const prompt = `Task: Generate **Full-Fidelity** DEXPI 2.0 equipment cards for the following list of equipment types:
${JSON.stringify(listOfTypesFromRegistry, null, 2)}
...
Return JSON:
{
  ...
  "recommendations": ["..."]
    ...
    private buildSystemPrompt(persona: PersonaName, context?: AgentContext): string {
        ...
        if (context) {
            if (persona === 'theEngineer' && context.listOfTypesFromRegistry) {
