  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-2xl font-bold text-white">Critical Infrastructure Sectors</h2>
      ...
      {showCreate && (
        <div className="card mb-6">
          <h3 className="text-lg font-semibold text-white mb-3">Create New Sector</h3>
          ...
          <div key={sector.code} className="card">
            <div className="flex items-center justify-between cursor-pointer" onClick={() => toggle(sector.code)}>
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-lg font-semibold text-white">{sector.code} — {sector.name}</h3>
              ...
              <div className="mt-4 space-y-3 border-t border-gray-800 pt-4">
                {sector.subSectors.map(sub => (
                  <div key={sub.code} className="pl-4">
                    <h4 className="text-sm font-medium text-gray-200">{sub.code} — {sub.name}</h4>
                    <p className="text-xs text-gray-500 mb-2">{sub.description}</p>
                    <div className="grid grid-cols-1 gap-2">
                      {sub.facilities.map(fac => (
                        <div key={fac.code} className="pl-4 p-2 bg-gray-800/50 rounded">
                          <div className="text-xs font-medium text-gray-300">{fac.code} — {fac.name}</div>
                          <div className="flex flex-wrap gap-1 mt-1">
                            {fac.equipment.slice(0, 6).map(eq => (
                              <span key={eq.componentClass} className={`text-xs px-1.5 py-0.5 rounded ${eq.category === 'rotating' ? 'bg-blue-900/30 text-blue-300' : eq.category === 'static' ? 'bg-green-900/30 text-green-300' : eq.category === 'instrumentation' ? 'bg-yellow-900/30 text-yellow-300' : eq.category === 'electrical' ? 'bg-purple-900/30 text-purple-300' : 'bg-gray-700 text-gray-300'}`}>
                                {eq.displayName}
                              </span>
                            ))}
