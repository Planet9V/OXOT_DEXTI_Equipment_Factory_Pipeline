...
const STATUS_COLORS: Record<string, string> = { pending: 'bg-gray-700', running: 'bg-blue-600 animate-pulse', completed: 'bg-green-600', failed: 'bg-red-600', skipped: 'bg-gray-600' };
...
export default function PipelinePage() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <h2 className="text-2xl font-bold text-white mb-6">AI Equipment Pipeline</h2>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Submit Form */}
        <div className="card">
          <h3 className="text-lg font-semibold text-white mb-4">Generate Equipment Cards</h3>
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mb-4">
                {activeRun.stages.map((stage, i) => (
                  <div key={stage.name} className="flex items-center">
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center text-lg ${STATUS_COLORS[stage.status] || 'bg-gray-700'}`}>
                      {STAGE_ICONS[stage.name]}
                    </div>
                    {i < activeRun.stages.length - 1 && (
                      <div className={`w-8 h-0.5 ${stage.status === 'completed' ? 'bg-green-600' : 'bg-gray-700'}`} />
                    )}
                  </div>
                ))}
              </div>
              ...
              {activeRun.results.stored > 0 && (
                <div className="grid grid-cols-4 gap-2 mb-3 text-center">
                  <div className="p-2 bg-gray-800 rounded"><div className="text-lg font-bold text-blue-400">{activeRun.results.generated}</div><div className="text-xs text-gray-500">Generated</div></div>
                  <div className="p-2 bg-gray-800 rounded"><div className="text-lg font-bold text-green-400">{activeRun.results.validated}</div><div className="text-xs text-gray-500">Validated</div></div>
                  <div className="p-2 bg-gray-800 rounded"><div className="text-lg font-bold text-purple-400">{activeRun.results.stored}</div><div className="text-xs text-gray-500">Stored</div></div>
                  <div className="p-2 bg-gray-800 rounded"><div className="text-lg font-bold text-yellow-400">{activeRun.results.duplicatesSkipped}</div><div className="text-xs text-gray-500">Skipped</div></div>
                </div>
              )}
              {/* Logs */}
              <div className="bg-gray-950 rounded-lg p-3 max-h-64 overflow-y-auto font-mono text-xs">
        ...
        <div className="card mt-6">
          ...
          <div className="space-y-2">
            {runs.slice(0, 10).map(run => (
              <div key={run.id} onClick={() => setActiveRun(run)} className="flex items-center justify-between p-3 bg-gray-800 rounded-lg cursor-pointer hover:bg-gray-750">
