...
describe('Facility Assignment', () => {
    ...
    test('assignEquipmentToFacility creates ASSIGNED_TO relationship', async () => {
        mockRunWrite.mockResolvedValueOnce({ counters: { updates: () => ({ relationshipsCreated: 1, nodesDeleted: 0 }) } });
    ...
    test('assignEquipmentToFacility returns false when not found', async () => {
        mockRunWrite.mockResolvedValueOnce({ counters: { updates: () => ({ relationshipsCreated: 0, nodesDeleted: 0 }) } });
    ...
    test('removeEquipmentFromFacility deletes relationship only', async () => {
        mockRunWrite.mockResolvedValueOnce({ counters: { updates: () => ({ relationshipsDeleted: 1, nodesDeleted: 0 }) } });
    ...
    test('removeEquipmentFromFacility returns false when no relationship', async () => {
        mockRunWrite.mockResolvedValueOnce({ counters: { updates: () => ({ relationshipsDeleted: 0, nodesDeleted: 0 }) } });
