...
describe('ProcurementAgent', () => {
    ...
    it('should throw error for wrapped JSON responses (object with variations key)', async () => {
        ...
        await expect(agent.execute({ equipment: mockEquipment }, 'test-run-id'))
            .rejects
            .toThrow('Output format invalid: Expected a JSON array of vendor variations.');
    });

    it('should throw error for wrapped JSON responses (object with models key)', async () => {
        const mockEquipment = { tag: 'TAG-001' } as unknown as EquipmentCard;
