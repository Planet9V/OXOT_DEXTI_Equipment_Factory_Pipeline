...
describe('Pipeline Batch Check API', () => {
    ...
    describe('POST /api/pipeline/batch-check', () => {
        test('should reject requests without items array', async () => {
            const body = {};
        ...
        test('should reject items array exceeding max size', async () => {
            const body = { items: Array.from({ length: 501 }, (_, i) => `Item ${i}`) };
            expect(body.items.length).toBeGreaterThan(500);
        });

        test('should deduplicate and trim input names', () => {
    ...
    describe('PipelineV2BatchParams', () => {
        test('should accept valid batch params', () => {
            const params = {
    ...
    describe('Pipeline API Route - Batch Mode Detection', () => {
        test('should detect batch mode when equipmentNames is present', () => {
            const body = {
        ...
        test('should detect legacy mode when equipmentNames is absent', () => {
            const body = {
                sector: 'CHEMICAL',
                subSector: 'REFINERY',
                facility: 'CDU',
        ...
        test('should reject batch with more than 100 items', () => {
    ...
    describe('Equipment Name Parsing', () => {
        ...
        test('should parse newline-separated names', () => {
            const raw = 'Pump\nValve\nCompressor';
            const names = [...new Set(raw.split(/[,;\n|]+/).map(s => s.trim()).filter(s => s.length > 0))];
            expect(names).toEqual(['Pump', 'Valve', 'Compressor']);
        });

        test('should parse semicolon-separated names', () => {
    ...
    describe('CSV Parsing', () => {
        test('should extract first column from CSV', () => {
