const fs = require('fs');
const content = fs.readFileSync('tests/agent.test.ts', 'utf8');

let newContent = content.replace("test('getPersonas returns all 6 personas', () => {", "test('getPersonas returns all 8 personas', () => {");
newContent = newContent.replace("expect(personas).toHaveLength(6);", "expect(personas).toHaveLength(8);");

const expectAnchor = "        expect(names).toContain('procurementOfficer');";
if (newContent.includes(expectAnchor)) {
    newContent = newContent.replace(expectAnchor, expectAnchor + "\n        expect(names).toContain('theSurveyor');\n        expect(names).toContain('theEngineer');");
}
...
const testsAnchor = "    test('findVendorVariations returns empty array on invalid JSON', async () => {";
